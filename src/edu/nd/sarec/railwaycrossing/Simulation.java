...
public class Simulation extends Application{
	...
	public void start(Stage stage) throws Exception {
		...
		// Train and Track Two
		RailwayTracks trackTwo = mapBuilder.getTrack("Indigo");
		Train trainTwo = new Train(trackTwo.getStartX()-100,trackTwo.getEndY()-25,Direction.EAST);
		root.getChildren().add(trainTwo.getImageView());
		
		
		for(CrossingGate gate: mapBuilder.getAllGates()) {
			train.addObserver(gate);
			trainTwo.addObserver(gate);
		}
		// Have the second road observe the first crossing gate
		mapBuilder.getGate("Gate2").addObserver(mapBuilder.getRoad("Western Highway"));
